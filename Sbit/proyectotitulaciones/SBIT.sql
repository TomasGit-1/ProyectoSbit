...
values(1,'LOPT980219HOCPRM07','LOPT980219HOCPRM07',1),
...
values(2,2,'FERNANDO MIGUEL','RAMOS','GOMEZ','framos-1m@hotmail.com','Iztaccihuatl #611-A Col.Volcanes',20,23,'Soltero','2016-08-21','RAGF950708HOCMMR00','H'),
...
insert into Mensajes(idMaestro,matricula,tipo,enviado,msgTexto,fecha) -- idMsg,idMaestro,matricula,tipo,enviado,msgTexto,msgArchivo,fecha
	values(1,103266,2,'R','Se le invita a la platica de becas manutencion y tranporte.','2018-11-24'),
    (2,103266,2,'R','Revision de tareas el dia 30 de Noviembre del 2018 para concluir con la materia.','2018-11-24'),
    (1,103266,2,'R','Tutoria','2018-11-24'),
    (4,103266,2,'R','Motivo regularizacion del curso matematicas III debido a la perdida de clases (Motivo de bloqueos) ','2018-11-24');
insert into Mensajes(idMaestro,matricula,tipo,enviado,msgTexto,msgArchivo,fecha)
    values(6,103266,2,'E','Tarea prezzi.(Ramos Gomez Fernando Miguel)','BLUES (Ramos Gomez Fernando Miguel).pptx','2018-11-24'),
    (8,103266,2,'E','Tarea Char Codes.(Ramos Gomez Fernando Miguel)','Char Codes (Key Codes).pdf','2018-11-24');
...
-- Vista de Alumno
CREATE OR REPLACE VIEW infoAlumno AS SELECT alu.matricula,pe.nombre,pe.apPat,pe.apMat,pe.correo,pe.domicilio,pe.idEstado,pe.edad,pe.edoCivil,pe.curp,pe.tipo,alu.EscProcedencia,alu.Statuss
FROM Personas pe inner join alumnos alu  on pe.idPersona=alu.idPersona ; 
-- Vista de Maestro
CREATE OR REPLACE VIEW infoMaestro AS SELECT pe.nombre,pe.apPat,pe.apMat,pe.correo,pe.edad,pe.curp,ma.tipo,ma.categoria,ma.numHoras
FROM Personas pe inner join Maestros ma  on pe.idPersona=ma.idPersona ; 
