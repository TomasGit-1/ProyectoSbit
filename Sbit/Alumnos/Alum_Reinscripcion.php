...
<?php
        if(isset($_POST['submit'])){
			print"<b><i id=\"Amarillo\">Puedes cursar las materias:</i></b></br>";
?>
            <table border=1>
                <!-- <caption style="color:red">Mapa curricular</caption> -->
            <form class="btn btn-outline-danger text-center" action="Alum_Horario.php" name="datos" method="POST">
<?php
        }else{
?>
            <table border=1>
                <!-- <caption style="color:red">Mapa curricular</caption> -->
            <form class="btn btn-outline-danger text-center" action="Alum_Reinscripcion.php" name="datos" method="POST">
<?php
		}
			$sum = 0;
			while($r<$i){
					...
					if($r>=$i){
						...
						if($AP>0){
							...
							if($RE>0){
								...
								if(isset($_POST['submit'])){
									if($sum<=50){
										print"<p id=\"Crema\">".$row->nombreMat." con: ".$row->creditos." creditos</p>";
										$sum+=$row->creditos;
									}
								}
								//-----------------------------------------------------------------------
		...
		if(isset($_POST['submit'])){
?>
            <input class="btn btn-outline-danger text-center" name="submit" id="Mostaza" type="submit" value="Generar Horario">
		</form>
<?php
        }else{
?>
            <input class="btn btn-outline-danger text-center" name="submit" id="Mostaza" type="submit" value="Materias a cursar">
		</form>
<?php
        }
?>
